            ...
            myChart.setOption({
              ...
              $.get('static/mock/index.json').done(function (data) {
                ...
                myChart.setOption({
                    ...
                    yAxis: {
                    max:(data.number.rooms.length-1),
                    data: data.number.rooms,
                    },
                    series: [{
                        // 根据名字对应到相应的系列
                        name: '使用人数',
                        data: data.number.data.map(function (item) {
                                return [item[1], item[0], item[2]];
                            })
                    }],
                    tooltip: {
                    position: 'top',
                    formatter: function (params) {
                        return data.number.hours[params.value[0]] +'有'+params.value[2] + '人在'+data.number.rooms[params.value[1]]+'开会 ';
                    }
                    }
                });
            });      
